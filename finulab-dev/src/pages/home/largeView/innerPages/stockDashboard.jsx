...
import {ArrowDropUp, AssuredWorkload, Brightness5Sharp, CalendarMonth, CalendarMonthOutlined, ChevronLeft, ChevronRight, DataObject, DisplaySettings, NightsStaySharp, PieChart, PostAdd, RemoveCircleOutline, Verified} from '@mui/icons-material';
...
import {set_dt_optns, setAvailable, setAvailableBank, selectEarningsCalendar} from '../../../../reduxStore/earningsCalendar';
...
export default function StockMarketDashboard_Home(props) {
    ...
    const earningsCalendar = useSelector(selectEarningsCalendar);
    ...
    const pullDashboardRankings = async (type, selection) => {
        ...
        if(!dataKeys.includes(selection) || dashboardState["type"] !== "stocks") {
    ...
    const [earningsCalendarSelectedDate, setEarningsCalendarSelectedDate] = useState("");
    useEffect(() => {
        const setEarningsCalendar = async () => {
            await generalOpx.axiosInstance.put(`/stock-market-data/earnings-calendar`, 
                {
                    "dt": earningsCalendar["available"]["selected"]
                }
            ).then(
                (response) => {
                    if(response.data["status"] === "success") {
                            ...
                            setAvailable(
                                {
                                    ...
                                    "selected": earningsCalendar["available"]["selected"],
                                    "dataLoading": false
                                }
                            )
                        );
            ...
            setEarningsCalendarSelectedDate(earningsCalendar["available"]["selected"]);
        }

        if(earningsCalendar["available"]["dataLoading"]) {
            setEarningsCalendar();
        }
    }, []);
    ...
    const pullMarketOverview = async () => {
        await generalOpx.axiosInstance.put(`/stock-market-data/stock-market-overview`, 
            {}
        ).then(
            (response) => {
                if(response.data["status"] === "success") {
    ...
    useEffect(() => {
        if(dashboardState["type"] !== "stocks") {
            dispatch(
                clearStockDashboardData("stocks")
            );
            pullDashboardRankings("primary", "marketCap");
        }
        
        if(dashboardNews["type"] !== "stocks") {
            dispatch(
        ...
        if(dashboardTradingActivity["page"] !== "stocks") {
            dispatch(
    ...
    const [disableEarningsCalendarNav, setDisableEarningsCalendarNav] = useState(false);
    const navigateThroughEarningsCalendar = (type) => {
        setDisableEarningsCalendarNav(true);
        const currentIndex = earningsCalendar["available"]["index"];
        ...
        if(type === "back") {
            if(currentIndex > 0) {
                dispatch(
                    setAvailable(
                        {
                            "index": currentIndex - 1,
                            "data": earningsCalendar["available"]["data"],
                            "selected": earningsCalendar["available"]["selected"],
                            "dataLoading": false
                        }
                    )
            ...
            const limit = Math.ceil(earningsCalendar["available"]["data"].length / 5);
        ...
        setDisableEarningsCalendarNav(false);
    }

    const [disableUpdateToCalendar, setDisableUpdateToCalendar] = useState(false);
    const updateEarningsCalendar = async (selected) => {
        setDisableUpdateToCalendar(true);

        if(earningsCalendar["availableBank"].some(avail_bnk => avail_bnk["selected"] === selected)) {
            const bankFiltered = earningsCalendar["availableBank"].filter(avail_bnk => avail_bnk["selected"] === selected)[0];
            ...
            await generalOpx.axiosInstance.put(`/stock-market-data/earnings-calendar`, 
                ...
                (response) => {
                    if(response.data["status"] === "success") {
                        dispatch(
                            setAvailableBank(
                                [...earningsCalendar["availableBank"], {...earningsCalendar["available"]}]
                            )
                        );
        ...
        setDisableUpdateToCalendar(false);
    }

    const earningsCalendarSelectedDateHandler = async (e) => {
        const {value} = e.target;
        setEarningsCalendarSelectedDate(value);

        const from = new Date(value);
        let selected = value, dt_optns = [];

        for(let i = 0; i < 11; i++) {
            const index = i - 2;
            const dt_add = add(from, {"days": index});
        
            dt_optns.push(format(dt_add, "yyyy-MM-dd"));
        }
        dispatch(
            set_dt_optns(dt_optns)
        );
        e.target.blur();

        await updateEarningsCalendar(value);
    }
            ...
            <div 
                        ...
                        <>
                            {dashboardState["page"]["data"][dashboardState["selection"]].slice(dashboardState["index"] * visibleElementsCount, (visibleElementsCount * (dashboardState["index"] + 1))).map((desc, index) => (
                                    <div className="large-stocksDashboardBodyLine" key={desc["symbol"]}>
                                        <button className={contentBodyWidth[0] < 605 ? "small-stocksDashboardBodyLineBtn" : "large-stocksDashboardBodyLineBtn"}
                                                                    ...
                                                                    <>
                                                                        {marketOverview["topFiftyGL"]["data"].slice(5 * marketOverview["topFiftyGL"]["index"], 5 * (marketOverview["topFiftyGL"]["index"] + 1)).map((txData, index) => (
                                                                                <button className={props.f_viewPort === "small" ? "small-stocksDashboardBodyLineBtn" : "large-stocksDashboardBodyLineBtn"}
                                                                                        onClick={() => navigate(`/stocks/S:-${txData.symbol}`)}
                                                                                    >
                                                                                    <div className="asset-congressTxsTableHeaderContainer"
                ...
                <div className="asset-dashboardEarningsCalendarContainer">
                    <div className="large-stocksNewsHeaderContainer"
                            style={{"height": "25px", "minHeight": "25px", "maxHeight": "25px"}}
                        >
                        <div className="large-stocksNewsHeader">Earnings Calendar</div>
                    </div>
                    <div className="asset-dashboardEarningsCalendarDateOptnsContainer">
                        <div className="asset-dashboardEarningsCalendarDateOptnsListContainer">
                            {earningsCalendar["dt_optns"].map((dt_optn, index) => {
                                    const dt_parsed = parse(dt_optn, "yyyy-MM-dd", new Date());
                                            
                                    const day_fWeek = format(dt_parsed, "EEE").toUpperCase();
                                    const date_fMonth = format(dt_parsed, "d MMM").toUpperCase();

                                    if(dt_optn === earningsCalendar["available"]["selected"]) {
                                        return <button className="asset-dashboardEarningsCalendarOptnBtn"
                                                disabled={disableUpdateToCalendar}
                                                onClick={() => updateEarningsCalendar(dt_optn)}
                                            >
                                            <div className="asset-dashboardEarningsCalendarOptnTopDesc"
                                                    style={{"color": "var(--primary-blue-10)"}}
                                                >
                                                {day_fWeek}
                                            </div>
                                            <div className="asset-dashboardEarningsCalendarOptnUnderDesc"
                                                    style={{"color": "var(--primary-blue-10)"}}
                                                >
                                                {date_fMonth}
                                            </div>
                                        ...
                                        return <button className="asset-dashboardEarningsCalendarOptnBtn"
                                                ...
                                                onClick={() => updateEarningsCalendar(dt_optn)}
                                            >
                                            <div className="asset-dashboardEarningsCalendarOptnTopDesc">{day_fWeek}</div>
                                            <div className="asset-dashboardEarningsCalendarOptnUnderDesc">{date_fMonth}</div>
                                        </button>
                                    }
                                })
                            }
                        </div>
                        <div className="asset-dashboardEarningsCalendarDatePickerDivider"/>
                        <div className="asset-dashboardEarningsCalendarDatePickerContainer">
                            <label htmlFor="stock-dashboard-earnings_datepicker" className="asset-dashboardEarningsCalendarDatePickerBtn">
                                <CalendarMonthOutlined className='asset-dashboardEarningsCalendarDatePickerBtnIcon' style={{"marginLeft": "20px"}}/>
                            </label>
                            <input type="date"
                                disabled={disableUpdateToCalendar}
                                id="stock-dashboard-earnings_datepicker"
                                value={earningsCalendarSelectedDate}
                                onChange={earningsCalendarSelectedDateHandler}
                                className="class_stock-dashboard-earnings_datepicker"
                            />
                        </div>
                    </div>
                    ...
                    <div className="finulab-tradingActivityChartTableContainer">
                        <div className="asset-congressTxsTableContainer">
                            <div className="asset-congressTxsTableSupport"
                                    style={{"minWidth": "550px", "minHeight": "315px"}}
                                >
                                <div className="asset-congressTxsTableHeaderContainer">
                                    <div className="asset-congressTxsTableHeaderTradeDate">
                                        <p className="asset-TopHoldersTableHeaderDesc">Time</p>
                                    </div>
                                    <div className="asset-congressTxsTableHeaderStock">
                                        <p className="asset-TopHoldersTableHeaderDesc">Symbol</p>
                                    </div>
                                    <div className="asset-congressTxsTableHeaderNamev5">
                                        <p className="asset-TopHoldersTableHeaderDesc">Name</p>
                                    </div>
                                    <div className="asset-congressTxsTableHeaderTradeRestv5">
                                        <p className="asset-TopHoldersTableHeaderDesc">Market Cap</p>
                                    </div>
                                    <div className="asset-congressTxsTableHeaderTradeRestv5">
                                        <p className="asset-TopHoldersTableHeaderDesc">Q-Ending</p>
                                    </div>
                                    <div className="asset-congressTxsTableHeaderTradeRestv5">
                                        <p className="asset-TopHoldersTableHeaderDesc">EPS EST</p>
                                    </div>
                                    <div className="asset-congressTxsTableHeaderTradeRestv5">
                                        <p className="asset-TopHoldersTableHeaderDesc">Last Yr-EPS</p>
                                    </div>
                                </div>
                                {earningsCalendar["available"]["dataLoading"] ?
                                    <>
                                        {Array(5).fill("").map((a, b) => (
                                                <div className="asset-congressTxsTableHeaderContainer"
                                                        style={b === 4 ? {"borderBottom": "none"} : {}}
                                    ...
                                    <>
                                        {earningsCalendar["available"]["data"].length > 0 ?
                                            <>
                                                {earningsCalendar["available"]["data"].slice(5 * earningsCalendar["available"]["index"], 5 * (earningsCalendar["available"]["index"] + 1)).map((txData, index) => (
                                                        <button className={props.f_viewPort === "small" ? "small-stocksDashboardBodyLineBtn" : "large-stocksDashboardBodyLineBtn"}
                                                                onClick={() => navigate(`/stocks/S:-${txData.symbol}`)}
                                                            >
                                                            <div className="asset-congressTxsTableHeaderContainer"
                                                                ...
                                                                <div className="asset-congressTxsTableHeaderTradeDate"
                                                                        style={{"height": "56px", "minHeight": "56px", "maxHeight": "56px"}}
                                                                    >
                                                                    <div className="asset-earningsCalendarTimeContainer">
                                                                        {txData.time === "time-not-supplied" ?
                                                                            <RemoveCircleOutline 
                                                                                style={{"color": "var(--primary-bg-05)"}}
                                                                            /> : 
                                                                            <>
                                                                                {txData.time === "time-pre-market" ? 
                                                                                    <Brightness5Sharp 
                                                                                        style={{"color": "var(--primary-amber-09)"}}
                                                                                    /> : 
                                                                                    <NightsStaySharp 
                                                                                        style={{"color": "var(--primary-bg-03)"}}
                                                                                    />
                                                                                }
                                                                            </>
                                                                        }
                                                                ...
                                                                <div className="asset-congressTxsTableHeaderNamev5"
                                                                        style={{"height": "56px", "minHeight": "56px", "maxHeight": "56px"}}
                                                                    >
                                                                    <p className="asset-congressTxsTableBodyTopDesc">{txData.name}</p>
                                                                </div>
                                                                <div className="asset-congressTxsTableHeaderTradeRestv5">
                                                                    <p className="asset-congressTxsTableBodyTopDesc" style={{"textAlign": "right"}}>
                                                                        {generalOpx.formatLargeFigures(txData.marketCap, 3)}
                                                                    </p>
                                                                </div>
                                                                <div className="asset-congressTxsTableHeaderTradeRestv5">
                                                                    <p className="asset-congressTxsTableBodyTopDesc" 
                                                                ...
                                                                <div className="asset-congressTxsTableHeaderTradeRestv5">
                                                                    <p className="asset-congressTxsTableBodyTopDesc"
                                                                            style={{"textAlign": "right"}}
                                                                        >
                                                                        {txData.epsForecast === "" ? `-` : `${txData.epsForecast}`}
                                                                    </p>
                                                                </div>
                                                                <div className="asset-congressTxsTableHeaderTradeRestv5">
                                                                    <p className="asset-congressTxsTableBodyTopDesc" style={{"textAlign": "right"}}>
                                                                        {txData.lastYearEPS >= 0 ?
                                                                            `$${generalOpx.formatFigures.format(txData.lastYearEPS)}` :
                                                                            `$(${generalOpx.formatFigures.format(Math.abs(txData.lastYearEPS))})`
                                                                        }
                                                                    </p>
                                                                </div>
                                                            </div>
                    ...
                    {earningsCalendar["available"]["dataLoading"] ?
                        null : 
                        <>
                            {earningsCalendar["available"]["data"].length > 5 ?
                                <div className="finulab-tradingActivityLatestTxRecordsContainer">
                                    {earningsCalendar["available"]["index"] + 1} | {Math.ceil(earningsCalendar["available"]["data"].length / 5)}
                                    <div className="large-stocksNewsViewToggleInnerContainer">
                                        <button className="asset-congressTxsViewMoreToggleBtn" 
                                                disabled={disableEarningsCalendarNav}
                                                onClick={() => navigateThroughEarningsCalendar("back")}
                                                style={earningsCalendar["available"]["index"] === 0 ? {"display": "none"} : {"display": "flex"}}
                                            >
                                            <ChevronLeft className="large-stocksNewsViewToggleOutlineIcon"/>
                                        </button>
                                        <div className="large-stocksNewsViewToggleOutlineDivider" 
                                        ...
                                        <button className="asset-congressTxsViewMoreToggleBtn" 
                                                disabled={disableEarningsCalendarNav}
                                                onClick={() => navigateThroughEarningsCalendar("forward")}
                                                style={earningsCalendar["available"]["index"] + 1 === Math.ceil(earningsCalendar["available"]["data"].length / 5) ? {"display": "none"} : {"display": "flex"}}
                                            >
                                            <ChevronRight className="large-stocksNewsViewToggleOutlineIcon"/>
                                        </button>
                                    </div>
